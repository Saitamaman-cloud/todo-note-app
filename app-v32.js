(function () {
  ...
  function hasRoutineTodoScheduleChanged(previousRoutine, nextRoutine) {
    const createSignature = (routine) => JSON.stringify({
      title: routine.title || "",
      repeatType: routine.repeatType || "manual",
      intervalDays: Number(routine.intervalDays || 1),
      daysOfWeek: Array.isArray(routine.daysOfWeek) ? [...routine.daysOfWeek].sort() : [],
      dayOfMonth: Number(routine.dayOfMonth || 1),
      startDate: routine.startDate || "",
      endDate: routine.endDate || "",
      nextDueDate: routine.nextDueDate || "",
      defaultTime: routine.defaultTime || "",
      isActive: routine.isActive !== false,
      autoCreateTodo: routine.autoCreateTodo === true
    });

    return createSignature(previousRoutine) !== createSignature(nextRoutine);
  }

  async function syncFutureRoutineTodos(routine) {
    const today = getTodayString();
    const allTodos = await window.TMTDB.getAllTodos();
    const relatedTodos = allTodos.filter((todo) => (
      todo.routineId === routine.id
      && todo.date
      && (todo.createdByRoutine === true || Boolean(todo.routineDate))
    ));
    const completedFutureDates = new Set(
      relatedTodos
        .filter((todo) => todo.date >= today && todo.status === "done")
        .map((todo) => todo.date)
    );
    const pendingFutureTodos = relatedTodos.filter((todo) => todo.date >= today && todo.status !== "done");
    const desiredDates = routine.autoCreateTodo && routine.isActive && routine.nextDueDate
      ? getRoutineTodoExpansionDates(routine)
      : [];
    const desiredDateSet = new Set(desiredDates);
    const existingByDate = new Map();
    const deleteIds = [];

    pendingFutureTodos.forEach((todo) => {
      if (!desiredDateSet.has(todo.date) || existingByDate.has(todo.date)) {
        deleteIds.push(todo.id);
        return;
      }
      existingByDate.set(todo.date, todo);
    });
    ...
    for (const todoId of deleteIds) {
      await window.TMTDB.deleteTodo(todoId);
    }
    ...
    for (const date of desiredDates) {
      ...
      if (existing) {
        const nextTime = routine.defaultTime || "";
        const needsUpdate = existing.title !== routine.title
          || (existing.time || "") !== nextTime
          || existing.routineDate !== date
          || existing.createdByRoutine !== true;

        if (needsUpdate) {
          await window.TMTDB.updateTodo({
            ...existing,
            title: routine.title,
            time: nextTime,
            routineDate: date,
      ...
      if (completedFutureDates.has(date)) {
        skipped += 1;
        continue;
      }

      const duplicate = allTodos.some((todo) => (
        todo.date === date
        && todo.title === routine.title
        && todo.routineId !== routine.id
      ));
      if (duplicate) {
        skipped += 1;
    ...
    return {
      ...
      skipped,
      preservedCompleted: completedFutureDates.size
    };
  }

  function formatRoutineTodoSyncSummary(result) {
    const parts = [
      `追加 ${result.added}件`,
      `更新 ${result.updated}件`,
      `旧予定の削除 ${result.removed}件`
    ];
    if (result.skipped) parts.push(`重複・完了済みの維持 ${result.skipped}件`);
    if (result.preservedCompleted) parts.push(`完了済みを保護 ${result.preservedCompleted}件`);
    return `（${parts.join("、")}）`;
  }
  ...
  async function handleSaveRoutine(event) {
    ...
    const previousRoutine = state.editingRoutine ? { ...state.editingRoutine } : null;
    ...
    try {
      ...
      if (previousRoutine && hasRoutineTodoScheduleChanged(previousRoutine, savedRoutine)) {
        const syncResult = await syncFutureRoutineTodos(savedRoutine);
        showMessage(`ルーチンを保存し、今日以降のToDoを同期しました。${formatRoutineTodoSyncSummary(syncResult)}`);
        navigate(savedRoutine.autoCreateTodo && savedRoutine.isActive ? "todo" : "routines");
        return;
      }
      ...
      console.error("Routine save or todo synchronization failed", error);
  ...
  async function toggleEditingRoutineActive() {
    ...
    try {
      const previousRoutine = { ...state.editingRoutine };
      const savedRoutine = await window.TMTDB.saveRoutine({
        ...previousRoutine,
        isActive: !previousRoutine.isActive
      });
      const syncResult = await syncFutureRoutineTodos(savedRoutine);
      state.editingRoutine = savedRoutine;
      fillRoutineForm(state.editingRoutine);
      elements.routineToggleActiveButton.textContent = state.editingRoutine.isActive ? "無効化" : "有効化";
      showMessage(`${state.editingRoutine.isActive ? "ルーチンを有効化しました。" : "ルーチンを無効化しました。"}${formatRoutineTodoSyncSummary(syncResult)}`);
    } catch (error) {
      console.error("Routine active state synchronization failed", error);
