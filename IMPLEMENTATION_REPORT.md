...
## v32 ルーチン予定同期（2026年8月9日）

ルーチン編集時に、今日以降のルーチン由来ToDoを新しい設定へ自動同期する処理を追加しました。

- 時刻・タイトル変更: 同じ日付の未完了ToDoを一括更新
- 曜日・間隔・毎月指定日・次回予定日・終了日変更: 旧予定を削除し、新しい日付へ再生成
- 自動作成の無効化・ルーチン無効化: 今日以降の未完了な自動生成ToDoを削除
- 自動作成の有効化・ルーチン有効化: 新しい設定で将来ToDoを生成
- 過去分、完了済み、手入力の通常ToDo、共有家事は変更しない
- IndexedDBはバージョン2のまま。Supabase SQL・RLSも変更なし
- PWAキャッシュを `today-memo-todo-cache-v32` へ更新
