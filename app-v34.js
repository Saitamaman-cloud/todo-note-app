(function () {
  ...
  const state = {
    ...
    editingNoteId: null,
    editingNote: null,
    noteDisplayMode: getSavedNoteDisplayMode(),
    isSavingNote: false,
        ...
        <main class="app-main" id="app">
          ...
          <section class="view is-active" id="view-home" aria-labelledby="home-title">
            ...
            <div class="summary-grid" aria-label="今日の件数">
              <button type="button" class="summary-card" data-home-status="todo">
                <span class="summary-label">未着手</span>
                <strong id="count-todo">0</strong>
              </button>
              <button type="button" class="summary-card" data-home-status="doing">
                <span class="summary-label">対応中</span>
                <strong id="count-doing">0</strong>
              </button>
              <button type="button" class="summary-card" data-home-status="done">
                <span class="summary-label">完了</span>
                <strong id="count-done">0</strong>
              </button>
              <button type="button" class="summary-card" data-home-status="routines">
          ...
          <section class="view" id="view-home-status" aria-labelledby="home-status-title">
            <div class="section-heading">
              <h2 id="home-status-title">今日のToDo</h2>
              <button type="button" class="secondary-button compact" data-go="home">ホームへ</button>
            </div>
            <p id="home-status-date" class="muted"></p>
            <div id="home-status-list" class="list"></div>
          </section>
          <section class="view" id="view-notes" aria-labelledby="notes-title">
            ...
            <div class="note-display-control">
              <label for="note-display-mode">一覧の表示</label>
              <select class="input" id="note-display-mode">
                <option value="preview">タイトルと本文</option>
          ...
          <section class="view" id="view-editor" aria-labelledby="editor-title">
            <div class="section-heading note-editor-toolbar">
              <div>
                <p class="muted">メモ編集</p>
                <h2 id="editor-title">メモを書く</h2>
              </div>
              <div class="note-editor-actions">
                <button class="primary-button compact" type="button" id="save-note-button">保存</button>
                <button class="secondary-button compact" type="button" id="back-to-notes">リスト</button>
              </div>
            </div>

            <dialog id="note-save-dialog" class="note-save-dialog" aria-labelledby="note-save-question" aria-describedby="note-save-description">
              <h3 id="note-save-question">保存しますか？</h3>
              <p id="note-save-description">「はい」は保存してリストへ戻ります。「いいえ」は今回の編集を保存せずに戻ります。</p>
              <p id="note-save-error" role="alert" hidden></p>
              <div class="button-row">
                <button class="primary-button" type="button" id="note-save-yes">はい</button>
                <button class="secondary-button" type="button" id="note-save-no">いいえ</button>
                <button class="secondary-button" type="button" id="note-save-cancel">編集を続ける</button>
              </div>
            </dialog>
            ...
            <form class="routine-editor-card" id="routine-form">
              <div class="form-grid">
                ...
                <div class="full">
                  ...
                  <p class="field-help">編集して保存すると、今日以降の未着手ToDoを新しい日時へ同期します。過去・対応中・完了済みは削除しません。</p>
                </div>
              </div>
  ...
  function cacheElements() {
    ...
    elements.noteDisplayMode = document.getElementById("note-display-mode");
    elements.noteDisplayMode.value = state.noteDisplayMode;
    elements.noteSaveDialog = document.getElementById("note-save-dialog");
    elements.noteSaveYes = document.getElementById("note-save-yes");
    elements.noteSaveNo = document.getElementById("note-save-no");
    elements.noteSaveCancel = document.getElementById("note-save-cancel");
    elements.noteSaveError = document.getElementById("note-save-error");
  ...
  function bindEvents() {
    document.querySelectorAll('[data-home-status]').forEach((button) => {
      button.addEventListener('click', () => {
        if (button.dataset.homeStatus === 'routines') {
          state.routineFilter = 'due';
          state.routineCategoryFilter = 'all';
          elements.routineCategoryFilter.value = 'all';
          navigate('routines');
        } else {
          navigate(`home-status-${button.dataset.homeStatus}`);
        }
      });
    });
    document.getElementById('home-status-list').addEventListener('click', handleTodoOpen);
    ...
    elements.backToNotes.addEventListener("click", () => {
      if (state.isSavingNote || elements.noteSaveDialog.open) return;
      elements.noteSaveError.hidden = true;
      elements.noteSaveDialog.showModal();
    });
    elements.noteSaveYes.addEventListener("click", () => handleSaveNote({ returnToList: true }));
    elements.noteSaveNo.addEventListener("click", () => {
      if (state.isSavingNote) return;
      elements.noteSaveDialog.close();
      navigate("notes");
    });
    elements.noteSaveCancel.addEventListener("click", () => elements.noteSaveDialog.close());
    elements.noteSaveDialog.addEventListener("cancel", (event) => {
      if (state.isSavingNote) event.preventDefault();
    });
    elements.noteDisplayMode.addEventListener("change", () => {
      state.noteDisplayMode = elements.noteDisplayMode.value === "title" ? "title" : "preview";
      try {
        localStorage.setItem("today-memo-todo-note-display-mode", state.noteDisplayMode);
      } catch (error) {
        showMessage("表示を切り替えました。この端末では表示設定を保存できませんでした。", true);
      }
      renderNotes();
    });
  ...
  function handleRoute() {
    ...
    if (/^home-status-(todo|doing|done)$/.test(route)) {
      showView('home-status');
      renderHomeStatus(route.replace('home-status-', ''));
      return;
    }
  ...
  function showView(viewName) {
    ...
    const activeNav = viewName === "home-status" ? "home" : viewName === "todo-detail" ? "todo" : viewName === "editor" ? "notes" : viewName === "routine-editor" ? "routines" : viewName;
  ...
  async function renderHome() {
    try {
      ...
      const [notes, routines] = await Promise.all([
        window.TMTDB.getAllNotes(),
        window.TMTDB.getAllRoutines()
      ]);
      await syncAutoRoutineTodos(routines);
      const todos = await window.TMTDB.getTodosByDate(today);
  ...
  async function renderNotes() {
    try {
      ...
      filteredNotes.forEach((note) => elements.noteList.append(createNoteButton(note, state.noteDisplayMode)));
    } catch (error) {
  ...
  function getSavedNoteDisplayMode() {
    try {
      return localStorage.getItem("today-memo-todo-note-display-mode") === "title" ? "title" : "preview";
    } catch (error) {
      return "preview";
    }
  }

  async function renderHomeStatus(status) {
    const list = document.getElementById('home-status-list');
    const today = getTodayString();
    document.getElementById('home-status-title').textContent = `今日の${STATUS_LABELS[status]}`;
    document.getElementById('home-status-date').textContent = formatDate(today);
    list.replaceChildren();
    try {
      const todos = (await window.TMTDB.getTodosByDate(today)).filter(todo => todo.status === status).sort(compareTodosByTime);
      // Do not render a stale response after switching between status lists.
      if (location.hash !== `#home-status-${status}`) return;
      if (!todos.length) list.append(createEmptyState(`今日の${STATUS_LABELS[status]}のToDoはありません。`));
      todos.forEach(todo => list.append(createTodoItem(todo)));
    } catch (error) {
      showMessage('今日のToDo一覧を読み込めませんでした。', true);
    }
  }

  async function handleSaveNote(options = {}) {
    if (state.isSavingNote) return;
    state.isSavingNote = true;
    const controls = [elements.saveNoteButton, elements.backToNotes, elements.noteSaveYes, elements.noteSaveNo, elements.noteSaveCancel];
    ...
    try {
      ...
      if (elements.noteSaveDialog.open) {
        elements.noteSaveError.textContent = "保存できませんでした。編集内容は残っています。もう一度お試しください。";
  ...
  function createNoteButton(note, displayMode = "preview") {
    const button = document.createElement("button");
    button.type = "button";
    ...
    if (displayMode === "title") {
      button.classList.add("note-item-title-only");
      button.append(title);
      return button;
    }
  ...
  async function buildRoutineTodoSyncPlan(routine) {
    const today = getTodayString();
    const allTodos = await window.TMTDB.getAllTodos();
    const relatedTodos = allTodos
      .filter((todo) => {
        if (todo.routineId !== routine.id || (todo.createdByRoutine !== true && !todo.routineDate)) {
          return false;
        }

        const scheduleDate = todo.routineDate || todo.date;
        return Boolean(
    ...
    const shouldReschedule = routine.isActive && routine.autoCreateTodo && routine.nextDueDate;
    const desiredDates = shouldReschedule ? getRoutineTodoExpansionDates(routine) : [];
    const desiredDateSet = new Set(desiredDates);
    const completedDates = new Set();
    const doingDates = new Set();
    ...
    relatedTodos.forEach((todo) => {
      const scheduleDate = todo.routineDate || todo.date;
      if (todo.status === "done") completedDates.add(scheduleDate);
      if (todo.status === "doing") doingDates.add(scheduleDate);
    });

    relatedTodos.forEach((todo) => {
      const scheduleDate = todo.routineDate || todo.date;
      const isExplicitManualOccurrence = Object.prototype.hasOwnProperty.call(todo, "routineAutoGenerated")
        && todo.routineAutoGenerated === false;

      if (todo.status === "done") return;

      if (isExplicitManualOccurrence) {
        manualCount += 1;
        if (desiredDateSet.has(scheduleDate)) retainedDates.add(scheduleDate);
        if (todo.title !== routine.title || String(todo.time || "") !== String(routine.defaultTime || "")) {
          updates.push({
            ...todo,
            title: routine.title,
      ...
      if (todo.status === "doing" || !shouldReschedule) {
        if (desiredDateSet.has(scheduleDate)) retainedDates.add(scheduleDate);
        if (todo.title !== routine.title || String(todo.time || "") !== String(routine.defaultTime || "")) {
          updates.push({
            ...todo,
      ...
      if (completedDates.has(scheduleDate) || doingDates.has(scheduleDate)) {
        deleteIds.push(todo.id);
        return;
      }

      if (desiredDateSet.has(scheduleDate) && !retainedDates.has(scheduleDate)) {
        retainedDates.add(scheduleDate);
        const nextTodo = {
          ...todo,
          date: scheduleDate,
          title: routine.title,
          time: routine.defaultTime || "",
          routineDate: scheduleDate,
          createdByRoutine: true,
          routineAutoGenerated: true
        };
        if (
          todo.date !== nextTodo.date
          || todo.title !== nextTodo.title
          || String(todo.time || "") !== String(nextTodo.time || "")
          || todo.routineDate !== nextTodo.routineDate
          || todo.createdByRoutine !== true
          || todo.routineAutoGenerated !== true
        ) {
          updates.push(nextTodo);
        }
        return;
      }
    ...
    const unrelatedTodoKeys = new Set(
      allTodos
        .filter((todo) => todo.routineId !== routine.id && todo.date)
        .map((todo) => `${todo.date}\u0000${todo.title || ""}`)
    );
    const additions = [];

    desiredDates.forEach((date) => {
      if (retainedDates.has(date) || completedDates.has(date) || doingDates.has(date)) return;
      if (unrelatedTodoKeys.has(`${date}\u0000${routine.title}`)) {
        conflictCount += 1;
        return;
      }
      additions.push({
    ...
    return {
      ...
      conflictCount,
      manualCount,
      protectedCount: completedDates.size + doingDates.size,
      rescheduled: Boolean(shouldReschedule)
    };
  }

  function getRoutineSyncMessage(plan) {
    const parts = [
      `変更 ${plan.updates.length}件`,
      `追加 ${plan.additions.length}件`,
      `旧予定の整理 ${plan.deleteIds.length}件`
    ];
    if (plan.conflictCount) parts.push(`同名ToDoとの重複 ${plan.conflictCount}件をスキップ`);
    if (plan.manualCount) parts.push(`手動追加 ${plan.manualCount}件を保護`);
    if (plan.protectedCount) parts.push(`対応中・完了済み ${plan.protectedCount}件を保護`);
    return `ルーチンを保存し、今日以降のToDoを同期しました（${parts.join("、")}）。`;
  }
  ...
  async function handleSaveRoutine(event) {
    ...
    try {
      const wasEditing = Boolean(state.editingRoutine?.id);
      const routineToSave = {
      ...
      if (wasEditing) {
        const plan = await buildRoutineTodoSyncPlan(routineToSave);
        const result = await window.TMTDB.saveRoutineWithTodoChanges(routineToSave, plan);
        savedRoutine = result.routine;
        showMessage(getRoutineSyncMessage(plan));
      } else {
        savedRoutine = await window.TMTDB.saveRoutine(routineToSave);
      }
      ...
      if (wasEditing) {
        if (savedRoutine.autoCreateTodo && savedRoutine.isActive && savedRoutine.nextDueDate) {
          setSelectedDate(getRoutineTodoDate(savedRoutine));
          navigate("todo");
        } else {
          navigate("routines");
        }
  ...
  function collectRoutineForm() {
    ...
    return {
      ...
      daysOfWeek,
      dayOfMonth,
      startDate: state.editingRoutine?.startDate && state.editingRoutine.startDate <= nextDueDate
        ? state.editingRoutine.startDate
        : nextDueDate,
      endDate,
  ...
  async function toggleEditingRoutineActive() {
    ...
    try {
      ...
      const plan = await buildRoutineTodoSyncPlan(routineToSave);
      const result = await window.TMTDB.saveRoutineWithTodoChanges(routineToSave, plan);
      state.editingRoutine = result.routine;
      fillRoutineForm(state.editingRoutine);
      elements.routineToggleActiveButton.textContent = state.editingRoutine.isActive ? "無効化" : "有効化";
      showMessage(`${state.editingRoutine.isActive ? "ルーチンを有効化しました。" : "ルーチンを無効化しました。"} ${getRoutineSyncMessage(plan)}`);
    } catch (error) {
